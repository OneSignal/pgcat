...
// let addrset = resolver.lookup_ip("www.example.com.").unwrap();
//
...
impl CachedResolver {
    //
    // Returns a new CachedResolver based on passed configuration.
    //
    // # Arguments:
    //
    ...
    // # Arguments
    //
    // * `mut& self` - Note that this method needs to be called using a mutable reference as
    //                 we need to modify the cache.
    ...
    // Fetches an AddrSet from the inner cache
    fn fetch_from_cache(&self, key: &str) -> Option<AddrSet> {
        CachedResolver::fetch_from_data_cache(self.data.clone(), key)
    }
    fn store_in_cache(data: Arc<RwLock<HashMap<String, AddrSet>>>, host: &str, addr_set: AddrSet) {
