...
pub struct ConnectionPool {
    ...
    pub cached_resolver: Option<Arc<CachedResolver>>,
}
...
impl ConnectionPool {
    ...
    pub async fn from_config(client_server_map: ClientServerMap) -> Result<(), Error> {
        ...
        let mut cached_resolver: Option<Arc<CachedResolver>> = None;
        if config.general.dns_cache_enabled {
            ...
            cached_resolver = match CachedResolver::new(cached_resolver_config).await {
                Ok(ok) => Some(ok),
                Err(err) => {
                    error!("Error Starting cached_resolver error: {:?}, will continue without this feature.", err);
                    None
                }
            };
        };
...
impl ManageConnection for ServerPool {
    ...
    async fn connect(&self) -> Result<Self::Connection, Self::Error> {
        ...
        let mut cached_resolver: Option<Arc<CachedResolver>> = None;
        if let Some(pool) = get_pool(&self.database, &self.user.username) {
            cached_resolver = pool.cached_resolver;
        }
